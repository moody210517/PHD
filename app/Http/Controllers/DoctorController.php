...
class DoctorController extends BaseController
{
    ...
    public function getPrestep2($id = '' , Request $request = null){
        ...
        if($patient){            
            ...
            $allocation_visit_form = AllocationVisitForm::where('patient_id', $patient_id)->where('tester_id', $tester_id)->orderBy('id', 'DESC')->get();
            ...
            if($user_diabet->first() && $allocation_visit_form->first()){
                ...
                ->with('visit_form_id', $allocation_visit_form->first()->id);
    ...
    public function getWaiting($id = '' , Request $request = null){
        ...
        if($visit_form->first()){
            ...
            if (strpos($treatment, '51') !== false) { // checked place marker in question
                $gsr = true;
            }            
        }

        if($patient->first()){
            return view('doctor.test.waiting_phd')
            ->with('patients', $patients)
    ...
    public function getPrestep3($id = '' , Request $request = null){
        ...
        $user_diabet_check = UserDiabet::where('user_id', $patient_id)->orderBy('id', 'DESC')->get();
    ...
    public function getCancelstep($id = '' , Request $request = null){

        $aid = $request->input('aid');
        DB::table('tbl_allocation')
        ->where('auto_num', $aid)
        ->update(['is_allocated' =>  2 ]);
        return view('doctor.testland');
